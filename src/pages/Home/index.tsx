...
export const HomePage = () => {
	...
	const filteredCars = allAnnouncements?.filter((announcement: any) => {
		return (
			...
			(!selectedPrice.min ||
				parseFloat(announcement.price) >=
					parseFloat(selectedPrice.min)) &&
			(!selectedPrice.max ||
	...
	const uniqueBrands = [
		...new Set(
			filteredCars?.map((announcement: any) => announcement.brand)
		),
	];
	const uniqueModels = [
		...new Set(
			filteredCars?.map((announcement: any) => announcement.model)
		),
	];
	const uniqueYears = [
	...
	const uniqueColors = [
		...new Set(
			filteredCars?.map((announcement: any) => announcement.color)
		),
	];
